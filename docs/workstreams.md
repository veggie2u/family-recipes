...
- ✅ "My stuff" section in UserMenu → `/recipes`, `/cookbooks`, `/families`
- ✅ Public `/cookbooks` listing page + `/cookbooks/[id]` detail page
- ✅ `/recipes`, `/cookbooks`, `/families` are auth-aware (authenticated → "my" content, unauthenticated → public listing)
- ✅ FeedCard source context is fully clickable (family name → `/families/{id}`, actor → `/profile/{id}`, cookbook → `/cookbooks/{id}`)
- ✅ Global search on feed: `search_global()` Postgres function (text match + relationship weight + recency decay) + debounced search bar in feed that replaces feed content with ranked results
