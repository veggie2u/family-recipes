...
import { addRecipeToFamily, removeRecipeFromFamily } from "@/app/dashboard/families/actions";
...
export default function AddRecipeToFamilyPanel({ familyId, allRecipes, initialSelectedIds }: Props) {
  ...
  const handleToggle = (recipeId: string) => {
    ...
    startTransition(async () => {
      try {
        if (isSelected) {
          await removeRecipeFromFamily(familyId, recipeId);
        } else {
          await addRecipeToFamily(familyId, recipeId);
        }
      } catch {
        setSelectedIds((prev) => {
          const next = new Set(prev);
